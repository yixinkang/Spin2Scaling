...
constexpr double GRID_SCALING_START = TOTAL_HOLD_TIME; // ms TOTAL_HOLD_TIME
// Define how often scaling happens (in ms)
const double SCALING_INTERVAL = 1e-4;  // Adjust this value as needed
...
double last_scaling_t = GRID_SCALING_START;  // Store last scaling time
...
uint integrateInTime(const double block_scale, const Vector3& minp, const Vector3& maxp)
{
	...
	std::string dirPrefix = "Testagain"+ dirSeparator + phaseToString(initPhase) + dirSeparator +
                        toString(SCALING_INTERVAL, 6) + "s_scaling" + dirSeparator + toStringShort(GRID_SCALING_START) + "ms_start" + dirSeparator +
	...
	while (t < STATE_PREP_DURATION) // only during STATE_PREP_DURATION
	{
		// update odd values
		t += dt / omega_r * 1e3; // [ms]
		signal = getSignal(t);
	...
	while (t < END_TIME) // main execution loop
	{
		...
		for (uint step = 0; step < IMAGE_SAVE_FREQUENCY; step++)
		{
			...
			if (t >= GRID_SCALING_START && (t - last_scaling_t) >= SCALING_INTERVAL)
			{
				...
				volume = expansionBlockScale * expansionBlockScale * BlockScale * VOLUME;
			...
			Bs.BqQuad = BqQuadScale * signal.Bq;
			Bs.BbQuad = BzQuadScale * signal.Bb;
			// std::cout << "Before odd leapfrog, N = " << getDensity(dimGrid, dimBlock, d_density, d_oddPsis[0], dimensions, bodies, volume) << std::endl;
			...
			// std::cout << "After odd leapfrog, N = " << getDensity(dimGrid, dimBlock, d_density, d_oddPsis[0], dimensions, bodies, volume) << std::endl;
			// update even values
			t += dt / omega_r * 1e3; // [ms]
			if (t >= GRID_SCALING_START && (t - last_scaling_t) >= SCALING_INTERVAL)
			{
			...
			Bs.BqQuad = BqQuadScale * signal.Bq;
			Bs.BbQuad = BzQuadScale * signal.Bb;
			// std::cout << "Before even leapfrog, N = " << getDensity(dimGrid, dimBlock, d_density, d_evenPsis[0], dimensions, bodies, volume) << std::endl;
			...
			// std::cout << "After even leapfrog, N = " << getDensity(dimGrid, dimBlock, d_density, d_evenPsis[0], dimensions, bodies, volume) << std::endl;
		}
#if SAVE_PICTURE
